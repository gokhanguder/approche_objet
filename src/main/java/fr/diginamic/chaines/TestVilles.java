...
public class TestVilles {
    ...
    public static void main(String[] args) {
        ...
        for (Ville ville : villes) {
            if (ville.getPopulation() > villePlusGrande.getPopulation()) {
                villePlusGrande = ville;
            }
        }

        System.out.println("\nLa plus grande ville : " + villePlusGrande.getNom() + " avec " + villePlusGrande.getPopulation() + " habitants");

        // Trouver la ville avec le plus petit nombre d'habitants
        Ville villePlusPetite = villes.get(0);
        for (Ville ville : villes) {
            if (ville.getPopulation() < villePlusPetite.getPopulation()) {
                villePlusPetite = ville;
            }
        }

        System.out.println("\nLa plus petite ville : " + villePlusPetite.getNom() + " avec " + villePlusPetite.getPopulation() + " habitants");
        ...
        for (Ville ville : villes) {
            System.out.println(ville.getNom() + " avec " + ville.getPopulation() + " habitants");
        }
